...
// ── Cursor-following dash cloud ────────────────────────────────────────────
// Antigravity colours: blue / violet / red
const C1 = { r: 49,  g: 139, b: 247 }; // #318bf7
const C2 = { r: 139, g: 110, b: 245 }; // #8b6ef5
const C3 = { r: 227, g: 80,  b: 88  }; // #e35058
...
function makeParticle() {
  return {
    ...
    orbitR:     80 + Math.random() * 400,    // wide spread — 80–480 px
    orbitSpeed: (0.00015 + Math.random() * 0.0003) * (Math.random() < 0.5 ? 1 : -1), // very slow
    breathAmp:  6 + Math.random() * 14,      // barely breathes
    breathFreq: 0.0006 + Math.random() * 0.001,
    breathPh:   Math.random() * Math.PI * 2,
    vx: 0, vy: 0,
    lag:  0.008 + Math.random() * 0.010,    // very lazy spring
    drag: 0.96,
    angle2:     Math.random() * Math.PI,
    spinRate:  (Math.random() - 0.5) * 0.0015, // nearly still
    len:        5 + Math.random() * 9,
    thickness:  1.2 + Math.random() * 1.3,
    col:        triColour(Math.random()),
    alpha:      0.10 + Math.random() * 0.13, // very subtle: 0.10–0.23
  };
}
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    // ── spring center that lazily chases the cursor ───────────────
    const cx = { val: 0, v: 0, target: 0 };
    const cy = { val: 0, v: 0, target: 0 };
    // Center spring: gentle catch-up, not launched
    const CENTER_K    = 0.025;  // gentler catch-up
    ...
    const resize = () => {
      ...
      // scatter existing particles over new canvas size
      particles.forEach(p => {
    ...
    // Raw cursor position for ripple proximity check
    ...
    // ── Ripple ring system ────────────────────────────────────────
    // A ripple is: { x, y, r, age, maxAge, maxR }
    const ripples = [];
    let rippleTimer = 0;
    const RIPPLE_INTERVAL = 90; // frames between auto-ripples when cursor is nearby
    ...
    const tick = () => {
      ...
      // Idle: wander on a Lissajous path
      if (idleMode) {
        cx.target = W * 0.5 + Math.sin(idleT * 1.1) * W * 0.20;
        cy.target = H * 0.5 + Math.sin(idleT * 0.7) * H * 0.15;
      }

      // Spring center toward target
      cx.v += (cx.target - cx.val) * CENTER_K;
      cx.v *= CENTER_DAMP;
      cx.val += cx.v;
      cy.v += (cy.target - cy.val) * CENTER_K;
      cy.v *= CENTER_DAMP;
      cy.val += cy.v;

      // ── Auto-emit ripple when cursor is inside the cloud ────────
      rippleTimer++;
      ...
      for (const p of particles) {
        ...
        // Ripple nudge: expanding wavefront pushes particles gently outward
        for (const rpl of ripples) {
          const rdx = p.x - rpl.x;
          const rdy = p.y - rpl.y;
          const d   = Math.sqrt(rdx * rdx + rdy * rdy) || 1;
          // Smooth pulse: bell curve around the wavefront radius
          const band = Math.exp(-0.5 * ((d - rpl.r) / 28) ** 2);
          // Fade in & out over ripple lifetime
  ...
  return (
    <div className="hp-shell">
      ...
      <nav className="hp-nav">
        <Link to="/" className="hp-nav-brand">
          <svg className="hp-nav-logo" viewBox="0 0 28 28" fill="none" aria-hidden="true">
            ...
            <polygon points="14,7 23,22 11,22" fill="#4285f4" opacity="0.7"/>
          </svg>
          Agent Native
        </Link>
      ...
      <section className="hp-hero">
        <canvas ref={canvasRef} className="hp-cloud-canvas" aria-hidden="true" />
        <div className="hp-hero-inner">
          <div className="hp-hero-brand">
            <svg width="22" height="22" viewBox="0 0 28 28" fill="none" aria-hidden="true">
              <polygon points="14,2 26,24 2,24" fill="#1a1a1a"/>
              <polygon points="14,7 23,22 11,22" fill="#4285f4" opacity="0.8"/>
            </svg>
      ...
      <section className="hp-features">
        ...
        <h2 className="hp-section-title">Built for accessibility.<br />Built for agents.</h2>
        <div className="hp-features-grid">
          {FEATURES.map(f => (
            <div className="hp-feat-card" key={f.title}>
              <div className="hp-feat-icon">{f.symbol}</div>
      ...
      <section className="hp-cta-band">
        ...
        <p className="hp-cta-sub">Make your React app accessible to every agent and user \u2014 in minutes.</p>
        <Link to="/toolkit" className="hp-btn-primary">Try the Toolkit</Link>
      </section>

      {/* Footer */}
      <footer className="hp-footer">
        <span className="hp-footer-brand">Agent Native</span>
        <span className="hp-footer-copy">IrvineHacks 2026</span>
      </footer>
