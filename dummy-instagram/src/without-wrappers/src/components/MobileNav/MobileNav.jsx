...
export default function MobileNav() {
    return (
        <nav className="mobile-nav">
            {[
                { to: '/', Icon: HomeIcon },
                { to: '/explore', Icon: SearchIcon },
                { to: '/reels', Icon: ReelsIcon },
                { to: '/notifications', Icon: HeartIcon },
                { to: '/profile/you.dev', Icon: ProfileIcon },
            ].map(({ to, Icon }) => (
                <NavLink key={to} to={to} className={({ isActive }) => `mobile-nav__link ${isActive ? 'mobile-nav__link--active' : ''}`} end={to === '/'}>
                    <Icon />
                </NavLink>
            ))}
        </nav>
