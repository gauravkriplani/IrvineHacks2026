    ...
    return (
        <div className="profile-page">
            ...
            <div className="profile-header">
                ...
                <div className="profile-info">
                    <div className="profile-info__row">
                        ...
                        {isOwn ? (
                            <>
                                <AOMAction id={`profile.${profileUser.username}.edit`} description="Edit your profile" safety={0.9} group="profile">
                                    <button className="profile-btn-outline">Edit profile</button>
                                </AOMAction>
                                <AOMAction id={`profile.${profileUser.username}.archive`} description="View your archive" safety={1} group="profile">
                                    <button className="profile-btn-outline">View archive</button>
                                </AOMAction>
                                <AOMAction id={`profile.${profileUser.username}.settings`} description="Open settings" safety={1} group="profile">
                                    <button className="profile-icon-btn"><SettingsIcon /></button>
                                </AOMAction>
                            </>
                        ) : (
                            <>
                                <AOMAction id={`profile.${profileUser.username}.toggle_follow`} description={`${following ? 'Unfollow' : 'Follow'} ${profileUser.username}`} safety={0.9} group="profile">
                                    <button
                                ...
                                <AOMAction id={`profile.${profileUser.username}.message`} description={`Message ${profileUser.username}`} safety={1} group="profile">
            ...
            <div className="profile-tabs">
                <AOMAction id={`profile.${profileUser.username}.tab_posts`} description="View posts tab" safety={1} group="profile">
                    <button className={`profile-tab ${activeTab === 'posts' ? 'profile-tab--active' : ''}`} onClick={() => setActiveTab('posts')}>
                        <GridIcon /> Posts
                    </button>
                </AOMAction>
                <AOMAction id={`profile.${profileUser.username}.tab_tagged`} description="View tagged tab" safety={1} group="profile">
            ...
            <div className="profile-grid">
                {(activeTab === 'posts' ? gridImages : gridImages.slice(0, 5)).map(item => (
                    <AOMAction key={item.id} id={`profile.${profileUser.username}.view_post.${item.id}`} description="View post details" safety={1} group="profile">
                        <div className="profile-grid__item" onClick={() => setSelectedPost(item)}>
